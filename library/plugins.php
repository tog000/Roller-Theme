...
/*
roller Plugins & Extra Functionality
...
function roller_get_socialimage() {
  global $post, $posts;
...
function roller_facebook_connect() {
	...
	echo '<meta property="og:image" content="'. roller_get_socialimage() .'"/>' . "\n";
	echo '<!-- end facebook open graph -->' . "\n";
}

// google +1 meta info
function roller_google_header() {
	if (is_singular()) {
		echo '<!-- google +1 tags -->' . "\n";
		global $post;
		echo '<meta itemprop="name" content="'.get_the_title().'">' . "\n";
		echo '<meta itemprop="description" content="' .strip_tags( get_the_excerpt() ).'">' . "\n";
		echo '<meta itemprop="image" content="'. roller_get_socialimage() .'">' . "\n";
		echo '<!-- end google +1 tags -->' . "\n";
	}
}
	
	// add this in the header 
	add_action('wp_head', 'roller_facebook_connect');
	add_action('wp_head', 'roller_google_header');
...
function roller_add_user_fields( $contactmethods ) {
	// Add Facebook
...
add_filter('user_contactmethods','roller_add_user_fields',10,1);
